...
var BackgroundCells = function (_React$Component) {
  ...
  BackgroundCells.prototype.isAvailableDateTime = function isAvailableDateTime(availibilityArray, slotInfo, isDayAvailibility, isMonthView) {
      ...
      range.map(function (date, index) {
        ...
        if (usersAvailability) {
          ...
          if (availableArray.unavailable && availableArray.unavailable.length) {
            var isDayAvailibility = false,
                isMonthView = true;
            isUnavailable = _this2.isAvailableDateTime(availableArray.unavailable, slotInfo, isDayAvailibility, isMonthView);
          }
          ...
          if (isUnavailable) {
            ...
            if (availableArray.available && availableArray.available.length) {
              var _isDayAvailibility = false,
                  _isMonthView = true;
              isAvailable = _this2.isAvailableDateTime(availableArray.available, slotInfo, _isDayAvailibility, _isMonthView);
            }

            if (!isAvailable && availableArray.days && availableArray.days.length) {
              var _isDayAvailibility2 = true,
                  _isMonthView2 = true;
              isAvailable = _this2.isAvailableDateTime(availableArray.days, slotInfo, _isDayAvailibility2, _isMonthView2);
            }
          }
        ...
        return _react2.default.createElement(
          ...
          _react2.default.createElement('div', {
            ...
            className: (0, _classnames2.default)('rbc-day-bg', selected && 'rbc-selected-cell', _dates2.default.isToday(date) && 'rbc-today', slot_bg_color)
          })
        );
      })
    );
  ...
  BackgroundCells.prototype._selectable = function _selectable() {
    ...
    selector.on('selecting', function (box) {
      ...
      if (!_this3.state.selecting) {
        (0, _helpers.notify)(_this3.props.onSelectStart, [box]);
        _this3._initial = { x: box.x, y: box.y };
      }
      if (selector.isSelected(node)) {
        var nodeBox = (0, _Selection.getBoundsForNode)(node);

        var _dateCellSelection = (0, _selection.dateCellSelection)(_this3._initial, nodeBox, box, range.length, rtl);
    ...
    selector.on('mousedown', function (box) {
      if (_this3.props.selectable !== 'ignoreEvents') return;

      return !(0, _Selection.isEvent)((0, _reactDom.findDOMNode)(_this3), box);
    });

    selector.on('click', function (point) {
      if (!(0, _Selection.isEvent)((0, _reactDom.findDOMNode)(_this3), point)) {
    ...
    selector.on('select', function () {
      _this3._selectSlot(_extends({}, _this3.state, { action: 'select' }));
      _this3._initial = {};
      _this3.setState({ selecting: false });
      (0, _helpers.notify)(_this3.props.onSelectEnd, [_this3.state]);
    });
  };
