...
var populateGroupAthletes = ($teamid) => {
	$(function(){
		var path = "/api/getathletesteam/"+$teamid;
		console.log('inside athleteCtrl, getAllAthletes');
		console.log($teamid);
		$.ajax({
	        ...
	        success: function(data) {
	            ...
	            for(var i = 0; i < athletes.length; i++) {
								$userid = athletes[i]._id;
								$( "#existingGroupModal .modal-body" ).append( "<div><a href='/views/athlete/" + $userid + "' value=" + athletes[i]._id + ">"  + athletes[i].profile.first+ ' ' + athletes[i].profile.last + "</option></div>" );
	        }
				}
		});
	});
...
var createGroup = () => {
	$(function(){
			...
			for (i = 0; i < athletesForGroup.length; i++)
  			$athletesForGroupString += (" \"" + athletesForGroup[i] + "\",");
			...
			$.ajax({
				...
				contentType: 'application/json',
	      	  url: server + path,						
						data: ("{ \"name\" : \"" + $groupname + "\" , \"athletes\" : [  " + $athletesForGroupString + "   ] }  "),
	        	success: function(data) {
								console.log(data)
	          	  console.log('created group');
						}
			});
			$athletesForGroupString = "";
			$(".modal-body").html("Group Created");
	});
}
